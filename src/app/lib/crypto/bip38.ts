...
export class BIP38 {
	static encrypt(privateKeyHex: string, passphrase: string, compressed = true): string {
		...
		if (keyBuf.length !== 32) { throw new Error('Invalid private key length'); }
	...
	static decrypt(bip38Str: string, passphrase: string): { privateKey: string; compressed: boolean } {
		...
		if (buf[0] !== HEADER) { throw new Error('Invalid BIP38 header'); }
		const type = buf[1];
		const flag = buf[2];
		if (type === TYPE.ECMULT) {
	...
	static verify(bip38Str: string): boolean {
		try {
			...
			if (![TYPE.ENC, TYPE.ECMULT].includes(buf[1])) { return false; }
			if (buf.length !== 39) { return false; }
			return true;
		} catch {
			return false;
		}
