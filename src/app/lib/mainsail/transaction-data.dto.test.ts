...
describe("TransactionData", () => {
	...
	beforeEach(() => {
		transaction = new TestTransactionData();
	...
	it("should be instantiated and configured", () => {
		expect(transaction.configure(commonData)).toBeInstanceOf(TestTransactionData);
		expect(transaction.raw()).toEqual(commonData);
	});

	it("should set decimals correctly", () => {
		expect(transaction.withDecimals(8)).toBeInstanceOf(TestTransactionData);
		expect(transaction.withDecimals("8")).toBeInstanceOf(TestTransactionData);
	});
