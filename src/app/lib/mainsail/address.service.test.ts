...
describe('AddressService', () => {
	...
	describe('fromMnemonic', () => {
		...
		it('should throw an error for an invalid mnemonic', () => {
			...
			expect(() => addressService.fromMnemonic(invalidMnemonic)).toThrow('The given value is not BIP39 compliant.');
		});
	});
	...
	describe('fromPrivateKey', () => {
		it('should return address from a private key', () => {
			const result = addressService.fromPrivateKey("cbf4b9f70470856bb4f40f80b87edb90865997ffee6df315ab166d713af433a5");
	...
	describe('fromSecret', () => {
		...
		it('should throw an error if the secret is BIP39 compliant', () => {
			expect(() => addressService.fromSecret(MAINSAIL_MNEMONICS[0])).toThrow('The given value is BIP39 compliant. Please use [fromMnemonic] instead.');
		});
	});
