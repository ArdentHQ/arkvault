...
describe("Money", () => {
	...
	it("should perform arithmetic operations", () => {
		...
		expect(money.plus(other).getAmount()).toBe(1500);
		expect(money.minus(other).getAmount()).toBe(500);
		expect(money.times(2).getAmount()).toBe(2000);
		expect(money.divide(2).getAmount()).toBe(500);
	});
