...
const Handle = ({ small, ...properties }: HandleProperties) => (
	<div
		{...properties}
		className={twMerge(
			"toggle-handle relative inline-flex h-[5px] rounded-full bg-theme-primary-100 dim:bg-theme-dim-700 dark:bg-theme-secondary-800",
...
const HandleInner = ({ alwaysOn, disabled, small, ...properties }: HandleInnerProperties) => (
	<span
		...
		className={twMerge(
			...
			cn({
				"bg-theme-primary-100 dim:bg-theme-dim-800 dark:bg-theme-secondary-800": disabled,
				"bg-theme-primary-600 dim:bg-theme-dim-navy-600": !disabled && alwaysOn,
				"bg-theme-secondary-400 dim:bg-theme-dim-500 dark:bg-theme-secondary-600": !disabled && !alwaysOn,
				"h-3 w-3": small,
				"h-4 w-4": !small,
