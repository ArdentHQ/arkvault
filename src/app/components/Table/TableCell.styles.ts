...
const getVariant = (isSelected: boolean, variant: "start" | "middle" | "end", size?: "sm" | "base"): string | null => {
	if (variant === "start") {
		const variants = {
			base: isSelected ? "pl-3 ml-3 bg-theme-success-100 rounded-l dark:bg-transparent dark:bg-transparent dark:border-y-2 dark:border-l-2 dark:border-theme-success-600" : "pl-6 rounded-l",
	...
	if (variant === "end") {
		const variants = {
			base: isSelected ? "pr-3 mr-3 bg-theme-success-100 rounded-r dark:bg-transparent dark:border-y-2 dark:border-r-2 dark:border-theme-success-600" : "pr-6 rounded-r",
