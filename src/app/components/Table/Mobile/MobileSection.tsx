...
export const MobileSection: FC<MobileSectionProps> = ({ className, title, children, ...props }) => (
	<div className={twMerge("flex w-36 flex-col gap-2", className)} {...props}>
		<span className="text-theme-secondary-700 dark:text-theme-secondary-500 text-sm font-semibold">{title}</span>

		<div className="text-theme-secondary-900 dark:text-theme-secondary-200 text-sm font-semibold">{children}</div>
	</div>
);
