...
describe("Password Settings", () => {
	...
	it("should trigger password confirmation mismatch error", async () => {
		...
		await userEvent.paste(screen.getByTestId(currentPasswordInputID), password);

		await waitFor(() => {
			expect(screen.getByTestId(currentPasswordInputID)).toHaveValue(password);
		});

		await userEvent.paste(passwordInput(), secondaryPassword);

		await waitFor(() => expect(passwordInput()).toHaveValue(secondaryPassword));

		await userEvent.paste(confirmPasswordInput(), "S3cUrePa$sword2different1");

		await waitFor(() => expect(confirmPasswordInput()).toHaveValue("S3cUrePa$sword2different1"));

		await userEvent.paste(passwordInput(), "new password 2");
	...
	it("should allow to remove the password", async () => {
		...
		await userEvent.paste(screen.getByTestId(confirmModalInputID), password);
	...
	it("should not allow password removal if current password does not match", async () => {
		...
		await userEvent.paste(screen.getByTestId(confirmModalInputID), "S3cUrePa$swordWrong");
