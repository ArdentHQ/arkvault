...
component.displayName = "ContractDeploymentForm";
transactionDetails.displayName = "ContractDeploymentFormTransactionDetails";

export const ContractDeploymentForm: SendRegistrationForm = {
	component,
	formFields: ["bytecode"],
	tabSteps: 2,
	transactionDetails,
...
export const signContractDeployment = async ({ env, form, profile, signatory }: any) => {
	...
	const transactionId = await senderWallet.transaction().signContractDeployment({
		...
		gasLimit,
		gasPrice,
		nonce: senderAddress.getAttributes().get('wallet.data.attributes.legacyNonce', undefined),
		signatory,
	});
