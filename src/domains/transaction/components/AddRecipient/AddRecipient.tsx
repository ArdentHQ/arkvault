...
export const AddRecipient: VFC<AddRecipientProperties> = ({
	...
	const { network, senderAddress, recipientAddress, amount, recipientAlias, isSendAllSelected } =
		watch();
	const { sendTransfer } = useValidation();

	const { minGasPrice } = getFeeMinMax();
	const fee = calculateGasFee(minGasPrice, GasLimit["transfer"].times(recipients.length));
