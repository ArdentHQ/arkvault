...
const fillFieldsWithValidAddressAndAmount = async (address: string, amount: string | number) => {
	...
	await userEvent.paste(amoutInput, String(amount));

	await waitFor(() => expect(amoutInput).toHaveValue(String(amount)));

	await userEvent.clear(addressInput);

	await userEvent.paste(addressInput, address);
...
describe("AddRecipient", () => {
	...
	it("should prevent adding invalid recipient address in multiple type", async () => {
		...
		await userEvent.paste(screen.getByTestId("AddRecipient__amount"), values.amount.toString());

		// Invalid address
		await userEvent.clear(screen.getByTestId("SelectDropdown__input"));
		await userEvent.paste(screen.getByTestId("SelectDropdown__input"), values.recipientAddress);
		...
		// Valid address
		await userEvent.clear(screen.getByTestId("SelectDropdown__input"));
		await userEvent.paste(screen.getByTestId("SelectDropdown__input"), "D8rr7B1d6TL6pf14LgMz4sKp1VBMs6YUYD");
	...
	it("should show error for low balance", async () => {
		...
		await userEvent.paste(screen.getByTestId("AddRecipient__amount"), "10000000000");
	...
	it("should show error for zero balance", async () => {
		...
		await userEvent.paste(screen.getByTestId("AddRecipient__amount"), "0.1");
	...
	it("should show error for invalid address", async () => {
		...
		await userEvent.paste(screen.getByTestId("SelectDropdown__input"), "abc");
