...
export const SendVoteSidePanel = ({
    ...
    const activeWallet = useMemo(
        () => (wallet ? wallet : activeProfile.wallets().findByAddressWithNetwork(getValues("senderAddress"), activeNetwork.id())),
    ...
    useEffect(() => {
        ...
        if (walletFromHook) {
            ...
            setValue("senderAddress", activeProfile.wallets().first().address(), { shouldDirty: true, shouldValidate: true });
        }
    ...
    const confirmSendVote = (wallet: Contracts.IReadWriteWallet, type: "unvote" | "vote" | "combined") =>
        new Promise((resolve) => {
            const interval = setInterval(async () => {
                ...
                if (type === "vote") {
                    isConfirmed = votes.length > 0 && walletVotes.some(({ wallet }) => wallet?.address() === votes[0].wallet?.address());
                }

                if (type === "unvote") {
                    isConfirmed = unvotes.length > 0 && !walletVotes.some(({ wallet }) => wallet?.address() === unvotes[0].wallet?.address());
                }

                if (type === "combined") {
                    const voteConfirmed = votes.length > 0 && walletVotes.some(({ wallet }) => wallet?.address() === votes[0].wallet?.address());
                    const unvoteConfirmed = unvotes.length > 0 && !walletVotes.some(({ wallet }) => wallet?.address() === unvotes[0].wallet?.address());
                ...
                if (senderWallet.network().votingMethod() === "simple") {
                    const { uuid, transaction } = await transactionBuilder.build(
                        "vote",
                        {
                            ...
                            data: {
                                unvotes: unvotes.map((unvote) => ({ amount: unvote.amount, id: unvote.wallet?.address() })),
            ...
            footer={
                <SidePanelButtons>
                    {activeTab !== Step.SummaryStep && (
                        <Button data-testid="SendVote__back-button" variant="secondary" onClick={handleBack} disabled={isSubmitting}>
                            {t("COMMON.BACK")}
                        </Button>
                    )}
                    ...
                    {activeTab === Step.AuthenticationStep && (
                        <Button data-testid="SendVote__send-button" onClick={() => void handleSubmit(onSubmit)()} disabled={isSubmitting}>
                            {t("COMMON.SEND")}
                        </Button>
                    )}
                ...
                <Tabs activeId={activeTab}>
                    <StepsProvider steps={MAX_TABS - 1} activeStep={activeTab}>
                        ...
                        <TabPanel tabId={Step.SummaryStep}>
                            {activeWallet && <TransactionSuccessful transaction={transaction!} senderWallet={activeWallet} noHeading />}
                        </TabPanel>
