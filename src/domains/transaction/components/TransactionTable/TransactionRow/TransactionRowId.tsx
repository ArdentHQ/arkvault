	...
	return (
		<div className="flex flex-col gap-1 font-semibold">
			...
			{!transaction.isSuccess() && transaction.blockHash() && (
				<Link to={transaction.explorerLink()} showExternalIcon={false} isExternal>
					<Tooltip content={transaction.hash()}>
						<span className="bg-theme-danger-50 dark:border-theme-danger-info-border dim:bg-transparent dim:border dim:border-theme-danger-info-border flex h-[21px] items-center space-x-1 rounded px-1.5 py-[2px] text-sm dark:border dark:bg-transparent">
							<TruncateMiddle
			...
			{!transaction.isSuccess() && !transaction.blockHash() && (
				<Link to={transaction.explorerLink()} showExternalIcon={false} isExternal>
					<Tooltip content={transaction.hash()}>
						<span className="bg-theme-secondary-200 dark:border-theme-dark-700 dim:bg-transparent dim:border dim:border-theme-dim-700 flex h-[21px] items-center space-x-1 rounded px-1.5 py-[2px] text-sm dark:border dark:bg-transparent">
							<TruncateMiddle
