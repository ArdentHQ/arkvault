...
describe("Welcome with page deeplink", () => {
	...
	it("should navigate to migration page", async () => {
		const { container } = render(
			<Route path="/">
				<Welcome />
			</Route>,
		...
		await waitFor(() => expect(history.location.pathname).toBe(`/profiles/${getDefaultProfileId()}/migration`));
	});

	it("should handle an invalid page", async () => {
		const toastErrorSpy = vi.spyOn(toasts, "error").mockImplementation(vi.fn());
		...
		await waitFor(() =>
			expect(toastErrorSpy).toHaveBeenCalledWith(
				result.current.buildSearchParametersError({ type: "INVALID_PAGE", value: "random" }),
			),
		);
