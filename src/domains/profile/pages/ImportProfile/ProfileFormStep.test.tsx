...
describe("Import Profile - Profile Form Step", () => {
	...
	it("should store profile", async () => {
		...
		inputElement.select();
		await userEvent.paste(inputElement, "test profile 1");
		...
		inputElement.select();
		await userEvent.paste(inputElement, "test profile 2");
	...
	it("should fail password confirmation", async () => {
		...
		await userEvent.paste(screen.getAllByTestId("Input")[0], "asdasdas");

		await userEvent.click(screen.getByTestId("SelectDropdown__caret"));
		await userEvent.click(screen.getByTestId("SelectDropdown__option--0"));
		await userEvent.click(screen.getByRole("checkbox"));

		await userEvent.paste(passwordInput(), "S3cUrePa$sword.test");
		await userEvent.paste(passwordConfirmationInput(), "S3cUrePa$sword.wrong");

		await waitFor(() => expect(submitButton()).toBeDisabled());

		passwordInput().select();
		await userEvent.paste(passwordInput(), "S3cUrePa$sword");

		passwordConfirmationInput().select();
		await userEvent.paste(passwordConfirmationInput(), "S3cUrePa$sword");

		await waitFor(() => expect(submitButton()).toBeEnabled());

		passwordConfirmationInput().select();
		await userEvent.paste(passwordConfirmationInput(), "S3cUrePa$sword.test");

		passwordInput().select();
		await userEvent.paste(passwordInput(), "S3cUrePa$sword.wrong");
	...
	it("should update the avatar when removing focus from name input", async () => {
		...
		inputElement.select();
		await userEvent.paste(inputElement, "t");
		...
		inputElement.select();
		await userEvent.paste(inputElement, "te");
		...
		inputElement.select();
		await userEvent.paste(inputElement, "test profile");
