...
import { ProfileCard2 } from "@/domains/profile/components/ProfileCard";
...
const ProfilesSlider = (properties: ProfilesSliderProperties) => {
	const settings: Settings = {
		appendDots: (dots) => (
			<div>
				<ul className="mt-3 flex justify-center gap-3 leading-3"> {dots} </ul>
			</div>
		),
		arrows: false,
		autoplay: false,
	...
	return (
		<Slider {...settings}>
			{profileChunks.map((profilesInChunk: Contracts.IProfile[], index) => (
				<ProfilesSlide profiles={profilesInChunk} {...rest} key={index} />
			))}
		</Slider>
	);
};
