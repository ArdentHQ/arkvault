  ...
  useEffect(() => {
    const checkHeight = () => {
      ...
      if (shouldUseScrollableForHeight && properties.profiles.length > PROFILES_PER_SLIDE) {
        ...
        const maxProfilesForHeight = Math.floor(Math.max(availableHeight, MIN_CONTAINER_HEIGHT) / PROFILE_ROW_HEIGHT);
...
const ScrollableProfiles = ({ 
  ...
  return (
    <div 
      className="space-y-3 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-theme-secondary-300 dark:scrollbar-thumb-theme-secondary-600" 
