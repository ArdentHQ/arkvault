...
const Slide = ({ title, subtitle, imageName, imageNameSm }: SlideProperties) => (
	<div className="flex w-full flex-row md:w-auto lg:flex-col">
		<div className="w-full p-4 sm:shrink-0 sm:p-6 md:w-[360px] md:pb-0 md:pr-0 lg:w-[450px]">
			<h3 className="mb-2 mt-7 text-base leading-7 dim:text-theme-dim-50 xs:leading-5 md:font-bold lg:text-2xl lg:leading-[29px]">
				{title}
			</h3>
			<p className="mb-0 text-sm text-theme-secondary-700 dim:text-theme-dim-200 dark:text-theme-secondary-500 md:mb-4 lg:text-base lg:leading-7">
				{subtitle}
			</p>
		</div>
		<div className="hidden flex-1 justify-end overflow-hidden rounded-ee-xl md:flex">
			...
			<Image
				...
				alt={title}
				domain="profile"
				className="hidden! md:block! lg:hidden! mt-3 h-[180px]"
			/>
		</div>
	</div>
);
