...
import { ImageWithPlaceholder } from "@/domains/profile/components/WelcomeSlider/ImageWithPlaceholder";
import {sliderLoadingImages} from "@/domains/profile/components/WelcomeSlider/sliderLoadingImages";
			...
			<div className="hidden flex-1 justify-end overflow-hidden rounded-ee-xl md:flex">
				<div className="hidden lg:block">
					<ImageWithPlaceholder
						...
						domain="profile"
						alt={title}
						wrapperClassName="aspect-[1.097]"
						fallbackImages={sliderLoadingImages[imageName] ?? sliderLoadingImages["WelcomeSlide1"]}
					/>
				</div>
				<div className="mt-3 hidden h-[180px] md:block lg:hidden">
					<ImageWithPlaceholder
						name={imageNameSm}
						domain="profile"
						alt={title}
						wrapperClassName="aspect-[1.6097]"
						imageClassName="h-[180px]"
						fallbackImages={sliderLoadingImages[imageNameSm] ?? sliderLoadingImages["WelcomeSlide1Sm"]}
					/>
				</div>
			</div>
		</div>
...
export const WelcomeSlider = (): JSX.Element => {
	const settings: Settings = {
		...
		customPaging: () => (
			<button
				className="h-3 w-3 rounded-full border-2 border-theme-navy-200 hover:border-transparent hover:bg-theme-navy-700 dark:border-theme-secondary-600 dark:hover:border-transparent dark:hover:bg-theme-secondary-600"/>
		),
		dots: true,
